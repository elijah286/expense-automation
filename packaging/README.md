...
These scripts produce a self-contained **Expense Automator** app (no separate Python install). The app matches `python -m web`: local server on port 8080 and your **default browser** by default (single Dock icon). Set `EXPENSE_AUTOMATOR_NATIVE=1` for an embedded pywebview window (may show extra Dock icons).
